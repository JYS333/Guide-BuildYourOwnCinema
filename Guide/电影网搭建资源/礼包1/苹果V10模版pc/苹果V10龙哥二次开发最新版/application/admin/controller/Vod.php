...
class Vod extends Base
{
    ...
    public function batch()
    {
        ...
        if (!empty($param)) {
            ...
            if(!empty($param['downer'])){
                $where['vod_downfrom'] = ['like','%'.$param['downer'].'%'];
            }
            ...
            foreach($res['list'] as  $k=>$v){
                ...
                if(!empty($param['ck_points']) && !empty($param['val_points_play'])  ){
                    $update['vod_points_play'] = $param['vod_points_play'];
                    $des .= '&nbsp;播放积分：'.$param['vod_points_play'].'；';
                }
                if(!empty($param['ck_points']) && !empty($param['val_points_down']) ){
                    $update['vod_points_down'] = $param['vod_points_down'];
                    $des .= '&nbsp;下载积分：'.$param['vod_points_down'].'；';
                }
                ...
                if($param['ck_del'] == 2 || $param['ck_del'] ==3){
                    ...
                    if($param[$par] == $v[$pre.'from']){
                        ...
                        $vod_from_arr = explode('$$$',$v[$pre.'from']);
                        $vod_server_arr = explode('$$$',$v[$pre.'server']);
                        $vod_note_arr = explode('$$$',$v[$pre.'note']);
                        $vod_url_arr = explode('$$$',$v[$pre.'url']);
                        ...
                        if($key!==false){
                            ...
                            $update[$pre.'from'] = join('$$$',$vod_from_arr);
                            $update[$pre.'server'] = join('$$$',$vod_server_arr);
                            $update[$pre.'note'] = join('$$$',$vod_note_arr);
                            $update[$pre.'url'] = join('$$$',$vod_url_arr);
                            $des .= '删除；';
                        }
                        else{
                            $des .= '跳过；';
