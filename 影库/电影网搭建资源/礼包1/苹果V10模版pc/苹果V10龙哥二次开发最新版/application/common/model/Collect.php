...
class Collect extends Base {
    ...
    public function vod_data($param,$data,$show=1)
    {
        ...
        $hitsstart = $config['vod']['hitsstart'];
        $hitsend = $config['vod']['hitsend'];
        $score = $config['vod']['score'];
        $inrule = $config['vod']['inrule'];
        $uprule = $config['vod']['uprule'];
        $hitsstart = $config['vod']['hitsstart'];
        $filter = $config['vod']['filter'];
        $thesaurus = $config['vod']['thesaurus'];
        $words = $config['vod']['words'];
        $status = $config['vod']['status'];
        $psernd = $config['vod']['psernd'];
        $psesyn = $config['vod']['psesyn'];
        $tag =  $config['vod']['tag'];
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif(strpos(','.$filter,$v['vod_name'])) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['vod_id']);
                ...
                if($hitsstart>0 && $hitsend>0) {
                    $v['vod_hits'] = rand($hitsstart, $hitsend);
                    $v['vod_hits_day'] = rand($hitsstart, $hitsend);
                    $v['vod_hits_week'] = rand($hitsstart, $hitsend);
                    $v['vod_hits_month'] = rand($hitsstart, $hitsend);
                }

                if($score==1) {
                    $v['vod_score_num'] = rand(1, 500);
                    $v['vod_score_all'] = $v['vod_score_num'] * rand(1, 10);
                    $v['vod_score'] = round($v['vod_score_all'] / $v['vod_score_num'], 1);
                }

                if ($psernd == 1) {
                    $v['vod_content'] = mac_rep_pse_rnd($words, $v['vod_content']);
                }
                if ($psesyn == 1) {
                    $v['vod_content'] = mac_rep_pse_syn($thesaurus, $v['vod_content']);
                }
                ...
                if (!$info) {
                    ...
                    if ($info['vod_lock'] == 1) {
                        ...
                        if (strpos(',' . $uprule, 'a') && !empty($v['vod_play_from'])) {
                        ...
                        if (strpos(',' . $uprule, 'b') && !empty($v['vod_down_from'])) {
                        ...
                        if (strpos(',' . $uprule, 'c') && !empty($v['vod_serial'])) {
                            $update['vod_serial'] = $v['vod_serial'];
                        }
                        if (strpos(',' . $uprule, 'd') && !empty($v['vod_remarks'])) {
                            $update['vod_remarks'] = $v['vod_remarks'];
                        }
                        if (strpos(',' . $uprule, 'e') && !empty($v['vod_director'])) {
                            $update['vod_director'] = $v['vod_director'];
                        }
                        if (strpos(',' . $uprule, 'f') && !empty($v['vod_actor'])) {
                            $update['vod_actor'] = $v['vod_actor'];
                        }
                        if (strpos(',' . $uprule, 'g') && !empty($v['vod_year'])) {
                            $update['vod_year'] = $v['vod_year'];
                        }
                        if (strpos(',' . $uprule, 'h') && !empty($v['vod_area'])) {
                            $update['vod_area'] = $v['vod_area'];
                        }
                        if (strpos(',' . $uprule, 'i') && !empty($v['vod_lang'])) {
                            $update['vod_lang'] = $v['vod_lang'];
                        }
                        if (strpos(',' . $uprule, 'j') && substr($info["vod_pic"], 0, 4) == "http") {
                            $update['vod_pic'] = $v['vod_pic'];
                        }
                        if (strpos(',' . $uprule, 'k') && !empty($v['vod_content'])) {
                            $update['vod_content'] = $v['vod_content'];
                        }
                        if (strpos(',' . $uprule, 'l') && !empty($v['vod_tag'])) {
                            $update['vod_tag'] = $v['vod_tag'];
                        }
                        if (strpos(',' . $uprule, 'm') && !empty($v['vod_sub'])) {
                            $update['vod_sub'] = $v['vod_sub'];
                        }
                        if (strpos(',' . $uprule, 'n') && !empty($v['vod_class'])) {
    ...
    public function art_data($param,$data,$show=1)
    {
        ...
        $hitsstart = $config['art']['hitsstart'];
        $hitsend = $config['art']['hitsend'];
        $score = $config['art']['score'];
        $inrule = $config['art']['inrule'];
        $uprule = $config['art']['uprule'];
        $hitsstart = $config['art']['hitsstart'];
        $filter = $config['art']['filter'];
        $thesaurus = $config['art']['thesaurus'];
        $words = $config['art']['words'];
        $status = $config['art']['status'];
        $psernd = $config['art']['psernd'];
        $psesyn = $config['art']['psesyn'];
        $tag =  $config['art']['tag'];
        ...
        foreach($data['data'] as $k=>$v){
            ...
            elseif(strpos(','.$filter,$v['art_name'])) {
                $des = '数据在过滤单中，跳过err';
            }
            else {
                unset($v['art_id']);
                ...
                if($hitsstart>0 && $hitsend>0) {
                    $v['art_hits'] = rand($hitsstart, $hitsend);
                    $v['art_hits_day'] = rand($hitsstart, $hitsend);
                    $v['art_hits_week'] = rand($hitsstart, $hitsend);
                    $v['art_hits_month'] = rand($hitsstart, $hitsend);
                }

                if($score==1) {
                    $v['art_score_num'] = rand(1, 500);
                    $v['art_score_all'] = $v['art_score_num'] * rand(1, 10);
                    $v['art_score'] = round($v['art_score_all'] / $v['art_score_num'], 1);
                }

                if ($psernd == 1) {
                    $v['art_content'] = mac_rep_pse_rnd($words, $v['art_content']);
                }
                if ($psesyn == 1) {
                    $v['art_content'] = mac_rep_pse_syn($thesaurus, $v['art_content']);
                }
                ...
                if (!$info) {
                    ...
                    if ($info['art_lock'] == 1) {
                        ...
                        if($rc){
                            ...
                            if(strpos(','.$uprule,'b')){ $update['art_author'] = $v['art_author']; }
                            if(strpos(','.$uprule,'c')){ $update['art_from'] = $v['art_from'];  }
                            if(strpos(','.$uprule,'d')){ $update['art_pic'] = $v['art_pic']; }
                            if(strpos(','.$uprule,'e')){ $update['art_tag'] = $v['art_tag']; }
