...
class Make extends Base
{
    ...
    public function type()
    {
        ...
        for($i=$start;$i<=$page_count;$i++){
            ...
            $this->label_type( $type_info['mid']==2 ? $GLOBALS['config']['view']['art_type'] : $GLOBALS['config']['view']['vod_type'] );
        ...
        if(!empty($type_ids)){
            ...
            if ($num>=count($type_ids)){
                if(empty($this->_param['jump'])){
                    ...
                    $this->echoLink('内容页生成完毕，稍后继续生成首页');
                    mac_jump( url('make/index',['jump'=>1]) ,3);
                    exit;
                }
            }
        ...
        foreach($res['list'] as $k=>$v){
            ...
            if(!empty($v['art_id'])) {
                ...
                for($i=1;$i<=$art_page_total;$i++){
                    ...
                    $this->buildHtml($link,'./', mac_tpl_fetch('art',$v['art_tpl'],'detail') );
                    if($i==1) {
                ...
                if($GLOBALS['config']['view']['vod_detail'] == 2){
                    $this->label_maccms();
                    $this->label_vod_detail($v, $GLOBALS['config']['view']['vod_detail']);
                    $link = mac_url_vod_detail($v);
                    $this->buildHtml($link, './', mac_tpl_fetch('vod', $v['vod_tpl'], 'detail'));
                ...
                foreach($flag as $f) {
                    ...
                    if ($GLOBALS['config']['view']['vod_'.$f] < 2) {
                        ...
                        if ($GLOBALS['config']['view']['vod_'.$f] == 2) {
                        	$_REQUEST['sid'] = 1;
                        	$_REQUEST['nid'] = 1;
                            $this->label_vod_play($f,$v,$GLOBALS['config']['view']['vod_'.$f]);
                            $link =  ($f=='play' ?mac_url_vod_play($v,['sid'=>1,'nid'=>1]) : mac_url_vod_down($v,['sid'=>1,'nid'=>1]) );
                            $this->buildHtml($link, './', mac_tpl_fetch('vod', $v['vod_tpl_'.$f], $f) );
                        ...
                        elseif ($GLOBALS['config']['view']['vod_'.$f] == 3) {
                            for ($i = 1; $i <= $v['vod_'.$f.'_total']; $i++) {
                                for ($j = 1; $j <= $v['vod_'.$f.'_list'][$i]['url_count']; $j++) {
                                    ...
                                    $this->buildHtml($link_sp[0], './', mac_tpl_fetch('vod', $v['vod_tpl_'.$f], $f) );
                                    if($i==1 && $j==1) {
