...
class Index extends Controller
{
    ...
    private function checkDir()
    {
        $items = [
            ...
            ['file', './application/extra/maccms.php', '读写', '读写', 'ok'],
            ['file', './application/extra/bind.php', '读写', '读写', 'ok'],
            ['file', './application/extra/version.php', '读写', '读写', 'ok'],
        ];
        foreach ($items as &$v) {
            if ($v[0] == 'dir') {// 文件夹
                if(!is_writable($v[1])) {
    ...
    private function checkFunc()
    {
        ...
        foreach ($items as &$v) {
            if(('类'==$v[3] && !class_exists($v[0])) || ('模块'==$v[3] && !extension_loaded($v[0])) || ('函数'==$v[3] && !function_exists($v[0])) || ('配置'==$v[3] && ini_get('always_populate_raw_post_data')==0)) {
                $v[1] = '不支持';
                $v[2] = 'no';
                session('install_error', true);
            }
