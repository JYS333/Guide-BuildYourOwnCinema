...
function mac_get_rndstr($length=32)
{
...
function mac_interface_type()
{
    ...
    if(empty($data)){
        ...
        foreach($type_arr as $k=>$v){
            ...
            $data['vodtype'][$from] = $to;
        }

        $type_arr = explode('#',$arttype);
        foreach($type_arr as $k=>$v){
            list($from, $to) = explode('=', $v);
            $data['arttype'][$from] = $to;
        }
...
function mac_play_list($vod_play_from,$vod_play_url,$vod_play_server,$vod_play_note,$flag='play')
{
    ...
    foreach($vod_play_from_list as $k=>$v){
        ...
        $urls = mac_play_list_one($vod_play_url_list[$k]);
...
function mac_play_list_one($url_one, $server_url=''){
    $url_list = array();
    ...
    foreach($array_url as $key=>$val){
        ...
        if ( empty($url) ) {
            ...
            $url_list[$key+1]['url'] = $server_url.$title;
        }else{
            $url_list[$key+1]['name'] = $title;
            $url_list[$key+1]['url'] = $server_url.$url;
        }
...
function mac_param_url(){
    ...
    $param['page'] = !empty($input['page']) ? intval($input['page']) : 1;
